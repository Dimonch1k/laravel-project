...
<form action="{{ $action }}" method="post" enctype="multipart/form-data" class="space-y-6">
    ...
    <div class="mb-4">
        <label for="description" class="block text-sm font-medium dark:text-gray-100 text-gray-700">Description</label>
        <textarea id="description" name="description"
            class="mt-1 block w-full px-3 py-2 dark:text-white dark:bg-gray-600 dark:border-gray-800 border border-gray-300 rounded-md shadow-sm focus:outline-none focus:ring-indigo-500 focus:border-indigo-500 sm:text-sm">{{ old('description', $category->description ?? '') }}</textarea>
        @error('description')
            <div class="mt-1 text-sm text-red-600">{{ $message }}</div>
        @enderror
    </div>
    ...
    <div class="mb-4">
        ...
        <div class="mt-2">
            <p class="text-sm dark:text-gray-100 text-gray-500">Upload a JPEG, PNG, JPG, GIF, BMP, SVG or WEBP file.</p>
        </div>
        @error('image')
            <div class="mt-1 text-sm text-red-600">{{ $message }}</div>
        @enderror
